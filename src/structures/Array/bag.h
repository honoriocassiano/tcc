...
public:
	...
	Bag(int s,
//			const std::function<
//					void(BAG_ELEMENT, llint&, llint&, llint&)>& e_func) {
...
//	int Count() const {
	...
	BAG_ELEMENT ChooseRandom() const {
		assert(Count() > 0);
		while (1) {
			llint random_int = llint(floor(drand48() * size));
...
//	std::function<void(BAG_ELEMENT, llint&, llint&, llint&)> extract_func;
