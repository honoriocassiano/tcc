...
struct SkyboxVertex {
	...
	SkyboxVertex(const float x, const float y, const float z, const float s,
			const float t) :
			position(x, y, z), texCoord(s, t) {
	}
};

struct SkyboxSide {
	SkyboxVertex v0, v1, v2, v3;
	SkyboxSide(const SkyboxVertex &v0, const SkyboxVertex &v1,
			const SkyboxVertex &v2, const SkyboxVertex &v3) :
...
Skybox::Skybox() :
	...
	static SkyboxSide sides[SKYBOX_SIDES] =
			{ SkyboxSide(SkyboxVertex(1, -1, 1, 0, 0),
					SkyboxVertex(1, -1, -1, 1, 0), SkyboxVertex(1, 1, -1, 1, 1),
					SkyboxVertex(1, 1, 1, 0, 1)), SkyboxSide(
					SkyboxVertex(-1, -1, -1, 0, 0),
					SkyboxVertex(-1, -1, 1, 1, 0), SkyboxVertex(-1, 1, 1, 1, 1),
					SkyboxVertex(-1, 1, -1, 0, 1)), SkyboxSide(
					SkyboxVertex(-1, 1, 1, 0, 0), SkyboxVertex(1, 1, 1, 1, 0),
					SkyboxVertex(1, 1, -1, 1, 1),
					SkyboxVertex(-1, 1, -1, 0, 1)), SkyboxSide(
					SkyboxVertex(1, -1, 1, 0, 0), SkyboxVertex(-1, -1, 1, 1, 0),
					SkyboxVertex(-1, -1, -1, 1, 1),
					SkyboxVertex(1, -1, -1, 0, 1)), SkyboxSide(
					SkyboxVertex(-1, -1, 1, 0, 0), SkyboxVertex(1, -1, 1, 1, 0),
					SkyboxVertex(1, 1, 1, 1, 1), SkyboxVertex(-1, 1, 1, 0, 1)),
					SkyboxSide(SkyboxVertex(1, -1, -1, 0, 0),
							SkyboxVertex(-1, -1, -1, 1, 0),
							SkyboxVertex(-1, 1, -1, 1, 1),
							SkyboxVertex(1, 1, -1, 0, 1)) };
	...
	for (int i = 0; i < SKYBOX_SIDES; i++) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, TEXTURE_SIZE, TEXTURE_SIZE, 0,
		...
		static const GLint uniformPermTex = glGetUniformLocation(
				ShaderManager::GetShaderProgramHandle(SKYBOX_TEXTURE),
				"permTex");
